...
# 使用環境變數讀取金鑰，確保與 Render 的設定對接
# 這裡對應你提供的 API Key 變數邏輯
apiKey = os.environ['GEMINI_API_KEY']
url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key={apiKey}"
...
def handle_message(message):
    try:
        ...
        if response.status_code == 200:
            ...
            BOT.reply_to(message, ai_text.strip())
        else:
